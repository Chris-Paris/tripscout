...
export function TravelForm({ onSubmit, isLoading, language, onReset }: TravelFormProps) {
  ...
  useEffect(() => {
    ...
    if (currentInterests.length > 0) {
      const selectedIndices = currentInterests.map(interest => 
        interestsTranslations[language === 'en' ? 'fr' : 'en'].indexOf(interest)
      );

      const translatedInterests = selectedIndices
        .filter(index => index !== -1)
        .map(index => interestsTranslations[language][index]);
