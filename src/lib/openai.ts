...
8. Response format must be exactly:
{
  "attractions": [
    ...
    const userPrompt = language === 'fr'
      ? `Générez 5 nouvelles attractions pour ${destination} qui correspondent aux intérêts suivants: ${interests.join(', ')}. Ne pas inclure ces attractions existantes: ${existingAttractions.join(', ')}. REPONDEZ UNIQUEMENT AVEC UN OBJET JSON CONTENANT UN TABLEAU 'attractions'.`
      : `Generate 5 new attractions for ${destination} that match these interests: ${interests.join(', ')}. Do not include these existing attractions: ${existingAttractions.join(', ')}. RESPOND ONLY WITH A JSON OBJECT CONTAINING AN 'attractions' ARRAY.`;
...
7. Response format must be exactly:
{
  "hiddenGems": [
    ...
    const userPrompt = language === 'fr'
      ? `Générez 5 nouveaux trésors cachés pour ${destination} qui correspondent aux intérêts suivants: ${interests.join(', ')}. Ces lieux doivent être authentiques et hors des sentiers battus. Ne pas inclure ces lieux existants: ${existingGems.join(', ')}. REPONDEZ UNIQUEMENT AVEC UN OBJET JSON CONTENANT UN TABLEAU 'hiddenGems'.`
      : `Generate 5 new hidden gems for ${destination} that match these interests: ${interests.join(', ')}. These places should be authentic and off the beaten path. Do not include these existing places: ${existingGems.join(', ')}. RESPOND ONLY WITH A JSON OBJECT CONTAINING A 'hiddenGems' ARRAY.`;
...
8. Response format must be exactly:
{
  "activities": [
    ...
    const userPrompt = language === 'fr'
      ? `Générez 5 nouvelles suggestions d'activités pour ${destination} qui correspondent aux intérêts suivants: ${interests.join(', ')}. Ces activités doivent être uniques et ne pas inclure: ${existingActivities.join(', ')}. REPONDEZ UNIQUEMENT AVEC UN OBJET JSON CONTENANT UN TABLEAU 'activities'.`
      : `Generate 5 new activity suggestions for ${destination} that match these interests: ${interests.join(', ')}. These activities should be unique and not include: ${existingActivities.join(', ')}. RESPOND ONLY WITH A JSON OBJECT CONTAINING AN 'activities' ARRAY.`;
